...
function HeroSection() {
    return (
        <section className="relative overflow-hidden border-b-2 border-ink-900 bg-ink-900 pb-16 pt-32 lg:pb-24 lg:pt-40">
            ...
            <div className="pointer-events-none absolute -right-32 top-20 h-[500px] w-[500px] rounded-full bg-crawfish/[0.06] blur-[120px]" />
            <div className="relative mx-auto max-w-7xl px-6 lg:px-10">
                <div className="flex flex-col gap-10 lg:flex-row lg:items-center lg:gap-16">
                    <div className="flex-1">
                        <div className="flex items-center gap-3">
                            <CrossedFlags className="h-8 w-8 text-white/80" />
                            <Eyebrow>Our Story</Eyebrow>
                        </div>
                        <h1 className="mt-4 font-display text-hero uppercase text-white">
                            Who the hell
                            <br />
                            is <span className="text-crawfish">Charlie T?</span>
                        </h1>
                    ...
                    <div className="flex shrink-0 justify-center lg:block">
                        <img
                            ...
                            className="h-40 drop-shadow-[0_0_60px_rgba(255,90,31,0.25)] lg:h-64"
                        />
                    </div>
                </div>
            </div>
...
function NumbersSection() {
    return (
        <section className="border-b-2 border-ink-900 bg-surface-100 py-16 lg:py-20">
            <div className="mx-auto max-w-7xl px-6 lg:px-10">
                <dl className="grid gap-px overflow-hidden rounded-xl border-2 border-ink-900 bg-ink-900 sm:grid-cols-2 lg:grid-cols-4">
                    {NUMBERS.map(({ value, label }) => (
                        <div key={label} className="bg-surface-50 p-8 text-center">
                            <dt className="font-display text-[clamp(2.5rem,5vw,3.5rem)] leading-none text-crawfish">
                                {value}
                            </dt>
    ...
    return (
        <section className="bg-surface-100 py-24 lg:py-32">
            <div className="mx-auto max-w-7xl px-6 lg:px-10">
                ...
                <div className="relative border-l-2 border-ink-900 pl-8 md:pl-12">
                    {TIMELINE_EVENTS.map(({ year, title, body }, index) => (
                        ...
                        >
                            <NumberPlate
                                size="sm"
                                className="absolute -left-[calc(2rem+24px)] top-0 md:-left-[calc(3rem+24px)]"
                            >
                                {index + 1}
                            </NumberPlate>
                            <span className="font-display text-xs uppercase tracking-[0.2em] text-crawfish">{year}</span>
    ...
    return (
        <section className="border-t-2 border-ink-900 bg-surface-200 py-24 lg:py-32">
            <div className="mx-auto max-w-7xl px-6 lg:px-10">
                ...
                <div className="grid gap-px overflow-hidden rounded-xl border-2 border-ink-900 bg-ink-900 md:grid-cols-3">
                    {VALUES.map(({ title, body }, index) => (
                        <div key={title} className="bg-surface-50 p-10">
                            <NumberPlate size="sm">{`0${index + 1}`}</NumberPlate>
                            <h3 className="mt-5 font-display text-lg uppercase tracking-wide text-ink-900">{title}</h3>
                    ...
                    {SOURCING.map(({ title, from, body }) => (
                        <div key={title} className="bg-ink-900 p-8 lg:p-10">
                            <div className="flex items-start justify-between gap-4">
                                ...
                                <span className="shrink-0 rounded-full border border-caution/40 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.2em] text-caution">
                                    {from}
                                </span>
                            </div>
    ...
    return (
        <section className="border-t-2 border-ink-900 bg-surface-100 py-24 lg:py-32">
            <div className="mx-auto max-w-4xl px-6 text-center lg:px-10">
                <div className="checker-band mx-auto mb-8 h-3 w-16 bg-[length:8px_8px] text-ink-900" aria-hidden="true" />
                ...
                <p className="mt-8 font-paint text-xl text-crawfish">&mdash; Charlie T, Founder</p>
                <div className="checker-band mx-auto mt-8 h-3 w-16 bg-[length:8px_8px] text-ink-900" aria-hidden="true" />
            </div>
        </section>
    )
}
